	...
	display() {
		...
		for (var i = 0; i < this.res.length; i++) {
			str += `<li class="listsItem" index="${this.res[i].goodsId}">
					  ...
					  <p class="title"><a href="details.html">${this.res[i].name}</a></p>
					  <p class="price"><a href="details.html">${this.res[i].price}</a></p>
					  <p class="operation"><button class="btn btn-default" type="button">立即购买</button>
					  <button class="btn btn-theme ml10" type="button"><em>加入购物车</em></button></p></div></li>`;
		}
...
var gd=document.querySelectorAll("#floorGoods");
for(var i=0;i<gd.length;i++){
new List({
	lists:gd[i],
	url: "http://localhost/floor/json/data.json"
})
}
})();